...
enum MessageIDs
{
	...
	MESSAGE_ID_REQUEST_LATEST_UPLOADS			= 6,	// The request for the list of the latest uploads on the server (client to server)
	MESSAGE_ID_REQUEST_LATEST_UPLOADS_USER		= 7,	// The request for the list of the latest uploads by a specific user on the server (client to server)
	MESSAGE_ID_LATEST_UPLOADS_LIST				= 8,	// The list of the latest uploads on the server (server to client)
	MESSAGE_ID_FILE_REQUEST						= 9,	// File Request (client to server)
	MESSAGE_ID_FILE_REQUEST_FAILED				= 10,	// File Request Failure message (server to client)
	MESSAGE_ID_FILE_SEND_INIT					= 11,	// File Send Initializer (two-way)
